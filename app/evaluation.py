...
def llm_grade_solution(
    ...
    prompt = """
        ...
        2) The newly suggested solution (generated by a model).

        Focus on MEANING and KEY STEPS, not wording.

        When comparing, follow this process:
        - First, understand the user's problem from the ticket subject and body.
        - Then, break down the REFERENCE solution into a short ordered list of key actions / steps.
        - Break down the SUGGESTED solution into a short ordered list of key actions / steps.
        - Compare the two lists:
        - Are the same main actions present?
        - Are important prerequisites, checks, or configuration steps included?
        - Is the order of steps acceptable for the type of task?
        - Does the suggested solution introduce any clearly wrong or harmful steps?
        - Ignore minor wording changes or small differences in formatting.
        ...
        - "similarity": a number between 0 and 1 indicating how close the suggested solution is to the reference in meaning and effectiveness.
            ...
            - 0.7–0.89 = very similar and acceptable; small differences or minor missing details but still likely to solve the issue.
            - 0.4–0.69 = partial match; captures some key ideas but misses important steps or details.
            - 0.1–0.39 = mostly incorrect, incomplete, or missing the main idea.
            - 0.0 = completely unrelated or clearly wrong / harmful.
        - "category": one of "good_match", "partial_match", "mismatch"
            - good_match  -> similarity >= 0.7
            - partial_match -> 0.4 <= similarity < 0.7
            - mismatch -> similarity < 0.4
        - "explanation": a short natural-language explanation that:
            - mentions the main similarity/difference in steps, and
            - highlights any important missing or extra steps.

        Be strict and consistent:
        - Do NOT give a high similarity score just because both solutions mention the same system or application.
        - Penalize missing critical steps (e.g., permissions, configuration, verification, communication to the user).
        - Penalize solutions that would not actually resolve the user's problem, even if they sound similar.
        """.strip()
